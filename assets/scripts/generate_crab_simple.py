#!/usr/bin/env python3
"""Deterministically generate ``assets/crab_simple.usda`` from measured hardware dimensions.

Single source of truth: ``parkour_tasks/crab_hexapod_task/mdp/crab_hex_dimensions.py``
(loaded directly by file path -- stdlib only, no Isaac/torch needed). Regenerate with:

    python3 assets/scripts/generate_crab_simple.py            # overwrites assets/crab_simple.usda
    python3 assets/scripts/generate_crab_simple.py --out /tmp/x.usda   # elsewhere (tests)
