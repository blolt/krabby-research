...
class ImuParseReason(enum.Enum):
    ...
    TEMP_READ_FAILED = "temp-read-failed"    # only temp non-finite; firmware NAN-poison
...
class ImuTelemetry:
    ...
    def from_tokens(
        ...
        # The firmware NAN-poisons only the temp field (token 7) when the
        # temperature read fails while accel/gyro are good, so attribute that
        # distinctly from a wiring/link fault that garbles accel/gyro.
        if not all(math.isfinite(v) for v in vals):
            if all(math.isfinite(v) for v in vals[:6]):
                return _drop(ImuParseReason.TEMP_READ_FAILED)
