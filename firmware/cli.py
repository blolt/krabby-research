...
# board is silent at DEFAULT_BAUD, so a board that predates the 250000 raise
...
def cmd_update(branch_or_port: Optional[str] = None, port_arg: Optional[str] = None) -> None:
    ...
    # M16 raised the MCU link from 115200 to 250000 baud. Firmware and host must
    # agree: a partially-reflashed fleet talks past itself (garbage/no telemetry)
    # and boards that boot without hearing both siblings fall to ROLE_UNKNOWN.
    print(
        "  NOTE: M16+ firmware uses 250000 baud. Flash ALL THREE boards and update\n"
