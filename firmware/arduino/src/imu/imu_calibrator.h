        ...
        for (uint8_t axis = 0; axis < 3; ++axis)
        {
            calibratedMeasurement.acceleration[axis] =
                MetersPerSecondSquared(
                    measurement.acceleration[axis].value -
            ...
            const RadiansPerSecond gyroBias =
                DegreesPerSecond(calibration_.gyroBiasDegreesPerSecond[axis])
                    .toRadiansPerSecond();
            calibratedMeasurement.angularRate[axis] = RadiansPerSecond(
                measurement.angularRate[axis].value - gyroBias.value);
        }
...
private:
        ...
        const ImuMeasurement &measurement)
    {
        ...
        for (uint8_t axis = 0; axis < 3; ++axis)
        {
            ...
            accumulator.sum[axis] =
                DegreesPerSecond(accumulator.sum[axis].value + rate.value);

            if (accumulator.sampleCount == 0 ||
                rate.value < accumulator.minimum[axis].value)
            {
                accumulator.minimum[axis] = rate;
            }

            if (accumulator.sampleCount == 0 ||
                rate.value > accumulator.maximum[axis].value)
            {
        ...
        ImuCalibrationRecord &record)
    {
        for (uint8_t axis = 0; axis < 3; ++axis)
        {
            if (accumulator.maximum[axis].value -
                    accumulator.minimum[axis].value >
                IMU_CAL_MAX_SPREAD_DPS)
            {
                return ImuCalibrationResult::MotionDetected;
            }

            record.gyroBiasDegreesPerSecond[axis] =
                accumulator.sum[axis].value / IMU_CAL_SAMPLES;
        }
