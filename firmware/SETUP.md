...
The IMU-attached row was captured 2026-07-14 (same solo-bench method, BMI270 +
OLED chained at 0x69/0x3D) on the restructured library patch (register-keyed
`pgm_read_byte`, see the patch section above) — that run is also the hardware
validation of the restructured patch: init clean, EEPROM cal reload, gravity
9.6–9.9 m/s². IMU-attached vs IMU-absent delta: **+0.25 ms mean, p95 lower** —
inside run-to-run noise (AC 1c satisfied).
