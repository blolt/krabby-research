    ...
    if not valid:
        ...
        return Measurement(
            ...
            text=(f"{parsed}/{len(samples)} lines carried a ;IMU segment but none were "
                  "valid — sensor absent or recovery did not complete"),
            ok=False,
        )
...
def reconnect(port: str) -> Measurement:
    """Prove unplug/replug recovery without reopening serial and resetting the Mega."""
    with mcu.open_port(port) as ser:
        print("     waiting for connected baseline...")
        baseline = mcu.collect(ser, 40)

        input("     UNPLUG only the IMU/Qwiic cable, then press Enter > ")
        disconnected = mcu.collect(ser, 40)

        print("     reconnect the IMU and move/rotate it continuously for about 6 s.")
        input("     press Enter once it is reconnected and already moving > ")
        recovered = mcu.collect(ser, 120)

    baseline_imus = [i for i in (mcu.imu_of(s) for s in baseline) if i is not None]
    disconnected_imus = [
        i for i in (mcu.imu_of(s) for s in disconnected) if i is not None
    ]
    recovered_imus = [i for i in (mcu.imu_of(s) for s in recovered) if i is not None]

    baseline_valid = [i for i in baseline_imus if i.valid]
    disconnected_invalid = [i for i in disconnected_imus if not i.valid]
    recovered_valid = [i for i in recovered_imus if i.valid]
    recovered_gyro = [axis for imu in recovered_valid for axis in imu.gyro_dps]
    peak_dps = max((abs(axis) for axis in recovered_gyro), default=0.0)

    ok = (
        len(baseline_valid) >= 20 and
        len(disconnected_invalid) >= 20 and
        len(recovered_valid) >= 20 and
        peak_dps > 20.0
    )
    return Measurement(
        values={
            "baseline_valid": len(baseline_valid),
            "disconnected_invalid": len(disconnected_invalid),
            "recovered_valid": len(recovered_valid),
            "recovered_peak_dps": peak_dps,
        },
        text=(
            f"baseline valid {len(baseline_valid)}/{len(baseline_imus)}; "
            f"disconnected invalid {len(disconnected_invalid)}/{len(disconnected_imus)}; "
            f"recovered valid {len(recovered_valid)}/{len(recovered_imus)}; "
            f"recovered peak |gyro| {peak_dps:.1f} deg/s "
            "(pass requires live motion above 20 deg/s)"
        ),
        ok=ok,
    )


def oled_reconnect(port: str) -> Measurement:
    """Keep serial open while the OLED is unplugged and reconnected."""
    with mcu.open_port(port) as ser:
        mcu.collect(ser, 40)
        input("     UNPLUG only the OLED, then press Enter > ")
        absent = mcu.collect(ser, 80)
        input("     RECONNECT the OLED, then press Enter and rotate the IMU > ")
        recovered = mcu.collect(ser, 120)

    answer = input("     did the OLED redraw without a Mega reset? [y/N] > ")
    recovered_imus = [i for i in (mcu.imu_of(s) for s in recovered) if i is not None]
    recovered_valid = [i for i in recovered_imus if i.valid]
    recovered_gyro = [axis for imu in recovered_valid for axis in imu.gyro_dps]
    peak_dps = max((abs(axis) for axis in recovered_gyro), default=0.0)
    telemetry_ok = len(absent) >= 40 and len(recovered) >= 80
    imu_ok = len(recovered_valid) >= 80 and peak_dps > 20.0
    visual_ok = answer.strip().lower().startswith("y")
    return Measurement(
        values={
            "absent_lines": len(absent),
            "recovered_lines": len(recovered),
            "recovered_imu_valid": len(recovered_valid),
            "recovered_peak_dps": peak_dps,
        },
        text=(f"telemetry while absent {len(absent)} lines; after reconnect "
              f"{len(recovered)} lines; valid IMU {len(recovered_valid)}/"
              f"{len(recovered_imus)}; peak |gyro| {peak_dps:.1f} deg/s; "
              f"OLED redraw confirmed: {visual_ok}"),
        ok=telemetry_ok and imu_ok and visual_ok,
    )
