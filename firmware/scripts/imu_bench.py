...
from firmware.interfaces.joint_telemetry import parse_telemetry_line  # noqa: E402

PREFIXES = ("UNKWN;", "FRONT;", "LEFT ;", "RIGHT;")
...
def cmd_flip(ser: serial.Serial, args: argparse.Namespace) -> None:
    ...
    peak_gyro = 0.0
    state = "?"
    for_gate = lambda az: "DOWN" if az < -3 else ("up" if az > 3 else "sideways")
    while time.time() - t0 < args.seconds:
        parsed = parse_telemetry_line(ser.readline().decode("utf-8", errors="replace"))
