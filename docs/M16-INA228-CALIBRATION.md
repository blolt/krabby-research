...
It is intentionally not implemented in M16.

AC 3i names a "per-board VBUS **offset** trim", and §5's procedure line says
"record offsets". §5's one ambiguous phrase — "offset/gain trim" — sits in the
same sentence as "at **a known reference voltage**", singular, and a single
reference point yields one equation in two unknowns: it can determine an offset
or a gain, not both. Two operating points would be required:
...
The physics points the same way. The INA228 bus-voltage LSB is a fixed,
factory-trimmed constant, so offset dominates the VBUS error term. Shunt
resistance carries real tolerance, which is why a multiplicative trim does exist
where it is needed — `packShuntCal`, Procedure B below.
...
```
;BATT <pack_v> <pack_i> <pack_w> <pack_charge> <batt_a> <batt_b> <divergence> \
      <pack_region> <pack_valid> <midpoint_valid>
```

- **Check `pack_valid` and `midpoint_valid` first — both must read `1`.** The
  frame is emitted every tick whether or not the monitors answered; when a
  validity byte is `0` that monitor's fields carry its last trustworthy reading,
  which will look plausible and will not be current. Calibrating against a stale
  field silently trims the wrong thing.
...
- `pack_region` is decided by Task 4's thresholds. Through M16 Task 3 it reads
  `0` (`NORMAL`) and is not part of this procedure.
