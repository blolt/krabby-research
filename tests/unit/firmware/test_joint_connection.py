...
# ---------------------------------------------------------------- wire growth
# The joint segment gained a tenth field for attachment evidence. Both lengths
# have to parse: a leader on the new image still forwards nine-field lines from
# a follower that has not been reflashed.
class TestAttachmentEvidenceField:
    ...
    def test_absent_field_reads_as_verified(self):
        # Absence means the sender cannot report evidence, not that it has none.
        jt = JointTelemetry.from_tokens(self.NINE.split())
        assert jt.attachment_verified is True

    def test_tenth_field_is_decoded(self):
        assert JointTelemetry.from_tokens((self.NINE + " 0").split()).attachment_verified is False
        assert JointTelemetry.from_tokens((self.NINE + " 1").split()).attachment_verified is True

    def test_eleven_fields_is_rejected(self):
        # A longer segment is a framing error, not a newer sender to tolerate.
        assert JointTelemetry.from_tokens((self.NINE + " 1 2").split()) is None

    def test_evidence_is_independent_of_connection(self):
        # nan position and unverified attachment are separate facts; a channel can
        # be filtered for position reasons while attachment has been confirmed.
