...
# Every malformed shape the parser must drop, with the reason it must record.
# ids name the real-world event that produces each shape.
MALFORMED_SEGMENTS = [
    pytest.param(
        JOINT_SEG, ImuParseReason.WRONG_TAG,
        id="joint-segment-fed-to-imu-parser",
    ),
    pytest.param(
        "", ImuParseReason.WRONG_TAG,
        id="empty-segment",
    ),
    pytest.param(
    ...
    pytest.param(
        # Non-finite anywhere in the segment drops the whole sample, even if
        # accel/gyro are fine and valid=1.
        "IMU 0.012 -0.034 9.807 0.0012 -0.0008 0.0003 nan 1",
        ImuParseReason.NON_FINITE_VALUE,
...
def _bare_sdk() -> KrabbyMCUSDK:
    """SDK with only the state _parse_joint_line touches; no serial port."""
    sdk = object.__new__(KrabbyMCUSDK)
    sdk.joints = {}
    sdk.imu = None
    sdk.last_cmd = {}
    sdk._last_debug_log_ts = 0.0
    sdk.parse_stats = ParseStats()
    sdk._last_parse_warn_ts = 0.0
    sdk._imu_stale_warned = False
    return sdk
...
class TestSdkParseErrorObservability:
    ...
    def test_warning_is_reemitted_after_throttle_window_expires(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="KrabbySDK"):
            sdk._parse_joint_line(MALFORMED_IMU_LINE)
            # Simulate the ~1 s throttle window having elapsed.
            sdk._last_parse_warn_ts = time.time() - 2.0
            sdk._parse_joint_line(MALFORMED_IMU_LINE)
