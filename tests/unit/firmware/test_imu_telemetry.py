...
class TestImuFromTokens:
    ...
    def test_temp_only_nonfinite_keeps_sample_with_nan_temp(self):
        ...
        assert imu.valid is True
        # Not a parse error: the stats aggregate stays untouched.
        assert stats.error_count == 0
        assert stats.last_reason is None
...
class TestLossyStreamResilience:
    ...
    def test_stored_offending_segment_is_length_bounded(self):
        ...
        # message is truncated to exactly _MAX_STORED_SEGMENT chars, keeping the
        # HEAD (the "IMU " tag + first payload chars) so the log still names the
        ...
        assert stats.last_segment == "IMU " + "9" * (_MAX_STORED_SEGMENT - 4)
...
class TestSdkImuStorage:
    ...
    def test_connect_resets_parse_accounting_and_imu_cache(self):
        # connect() gives every connection a clean slate: fresh ParseStats,
        # cleared IMU cache, and a re-armed one-shot stale warning. Seed all
        # three with pre-connect traffic, then prove connect() wipes them.
        sdk = _bare_sdk()
        sdk._parse_joint_line(MALFORMED_IMU_LINE)   # parse error recorded
        sdk._parse_joint_line(STALE_LEADER_LINE)    # imu cached + stale-warn armed

        assert sdk.parse_error_count == 1
        assert sdk.last_parse_reason is not None
        assert sdk.imu is not None
        ...
        assert sdk.last_parse_reason is None
        assert sdk.imu is None
