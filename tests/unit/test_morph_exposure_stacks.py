...
class TestPlants:
    def test_golden_has_no_override_and_variants_resolve(self, m):
        assert "KRABBY_HEX_USD_PATH" not in m.p1_stack("base")
        for cfg in m.ORDER:
            if cfg == "base":
                continue
    ...
    def test_with_plant_replaces_a_stale_override(self, m):
        ...
        assert "KRABBY_HEX_USD_PATH" not in m.with_plant({"KRABBY_HEX_USD_PATH": "/stale.usda"}, "base")
