...
// The SparkFun 1.3" OLED inherits a constructor that skips QwGrSSD1306's member
// initializers, so like the production global these objects live in zeroed storage.
template <typename T>
class Zeroed
{
public:
    explicit Zeroed(TwoWire &wire) : storage_(new unsigned char[sizeof(T)]()), object_(new (storage_.get()) T(wire)) {}
    ~Zeroed() { object_->~T(); }
    Zeroed(const Zeroed &) = delete;
    Zeroed &operator=(const Zeroed &) = delete;
...
{
    Zeroed<Ssd1306Canvas> canvas(testWire);
...
{
    Zeroed<Ssd1306Adapter> adapter(testWire); TEST_ASSERT_FALSE(adapter->isInitialized());
...
{
    Zeroed<Ssd1306Adapter> adapter(testWire); DisplayFrame frame; init(*adapter);
    ...
    Zeroed<Ssd1306Adapter> fresh(testWire); init(*fresh);
    TEST_ASSERT_TRUE(environment.panel.ram() != changed);
...
{
    Zeroed<Ssd1306Adapter> adapter(testWire); DisplayFrame frame; init(*adapter); TEST_ASSERT_TRUE(adapter->render(frame));
...
static void test_failed_reset_and_retry_boundary_with_rollover()
{
    for (uint64_t start : {uint64_t(0), uint64_t(UINT32_MAX)-499})
    {
        ...
        Zeroed<Ssd1306Adapter> adapter(testWire); DisplayFrame frame; init(*adapter,false); environment.events.clear();
...
{
    Zeroed<Ssd1306Adapter> adapter(testWire); DisplayFrame frame; waitTwo(*adapter,frame);
...
static void test_timeout_recovery_on_free_bus()
{
    ...
    Zeroed<Ssd1306Adapter> adapter(testWire); DisplayFrame frame; init(*adapter,false);
