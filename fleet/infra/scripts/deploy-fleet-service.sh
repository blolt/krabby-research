...
# App code + systemd units are pushed via SSM on every deploy (no SSH on the
# box). Shared with fleet-deploy.yml — see push-fleet-apps.sh.
echo
"$INFRA_DIR/scripts/push-fleet-apps.sh"
